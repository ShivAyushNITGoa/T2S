...
import { JourneyModule, VideoArchive, LibraryBook, UserProfile, ShopProduct } from '../types';
import { Plus, Edit2, Trash2, X, Save, Film, Book, Map, Zap, ShoppingCart, ArrowLeft, Check, Upload, Search, Crown, ShieldAlert, ShieldCheck } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';

export default function AdminPanel() {
  const [activeSubTab, setActiveSubTab] = useState<'journey' | 'archives' | 'library' | 'users' | 'shop'>('journey');
...
function ArchiveManager() {
  ...
  const save = async (e: any) => {
    e.preventDefault();
    const inputUrl = e.target.thumbnailUrl?.value?.trim() || '';
    const finalThumbnail = thumbnailBase64 || inputUrl || 'https://images.unsplash.com/photo-1518770660439-4636190af475?auto=format&fit=crop&q=80&w=600';
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <div 
                  ...
                  className="border border-dashed border-white/15 rounded-2xl p-4 bg-white/5 hover:bg-white/10 hover:border-amber-500/30 transition-all text-center cursor-pointer relative group flex flex-col items-center justify-center min-h-[120px]"
                >
                  {thumbnailBase64 ? (
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs font-bold text-white/70">Upload Thumbnail File</p>
                ...
                <div>
                  <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest block my-1 px-1 font-mono">OR Thumbnail Image URL</label>
                  <input name="thumbnailUrl" defaultValue={editing.thumbnail} placeholder="https://..." className="w-full bg-white/5 p-3 rounded-xl text-sm text-white placeholder-gray-500 border border-white/5 focus:border-amber-500/30 focus:bg-white/10 focus:outline-none transition-all font-mono" />
                </div>
              </div>
              <div>
...
function LibraryManager() {
  ...
  const save = async (e: any) => {
    e.preventDefault();
    const inputCoverUrl = e.target.coverUrl?.value?.trim() || '';
              ...
              <div className="space-y-2">
                ...
                >
                  {coverBase64 ? (
                    ...
                    <div className="space-y-1">
                      <Upload className="w-6 h-6 text-white/45 mx-auto mb-1 animate-pulse" />
                      <p className="text-xs font-bold text-white/70">Upload Book Cover File</p>
                      <p className="text-[8px] text-white/30 uppercase font-black tracking-widest font-mono">JPG, PNG, WEBP from device</p>
                    </div>
                  )}
                  <input 
                ...
                <div>
                  <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest block my-1 px-1 font-mono">OR Cover Image URL</label>
                  <input name="coverUrl" defaultValue={editing.coverUrl} placeholder="https://..." className="w-full bg-white/5 p-3 rounded-xl text-sm text-white placeholder-gray-500 border border-white/5 focus:border-amber-500/30 focus:bg-white/10 focus:outline-none transition-all font-mono" />
                </div>
              </div>
              <div>
...
function UserManager() {
  ...
  const [filter, setFilter] = useState<'all' | 'pending' | 'premium' | 'standard'>('all');
  ...
  const handleApprove = async (uid: string, name?: string) => {
    try {
      ...
      setActionStatus(`Granted Premium Access to ${name || 'User'}`);
    } catch (e) {
      handleFirestoreError(e, OperationType.UPDATE, `users/${uid}`);
    }
  };

  const handleDecline = async (uid: string, name?: string) => {
    try {
      await updateDoc(doc(db, 'users', uid), {
        isStrategist: false,
        premiumRequestStatus: 'denied'
      });
      setActionStatus(`Denied request for ${name || 'User'}`);
    } catch (e) {
      handleFirestoreError(e, OperationType.UPDATE, `users/${uid}`);
    }
  };
            ...
            {pendingUsers.map(u => (
              <div key={u.uid} className="p-5 bg-gradient-to-r from-amber-500/[0.02] via-neutral-900 to-neutral-900 border border-amber-500/30 rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-md">
                ...
                <div className="flex gap-2.5 w-full md:w-auto pt-2 md:pt-0 self-end md:self-center">
                  ...
                  <button 
                    onClick={() => handleDecline(u.uid, u.displayName)}
      ...
      <div className="space-y-4 pt-6 border-t border-white/5">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="relative w-full sm:w-72">
            ...
            <input 
              ...
              className="w-full bg-white/5 border border-white/10 rounded-xl pl-9 pr-8 py-2 text-xs text-white placeholder-zinc-500 focus:outline-none focus:border-amber-500/50 font-mono transition-all"
            />
            {searchTerm && (
              <button 
        ...
        <div className="flex flex-wrap gap-2 text-xs font-mono font-bold">
          <button 
            ...
            className={`px-3 py-1.5 rounded-xl transition-all ${filter === 'all' ? 'bg-white text-black font-black' : 'bg-white/5 text-zinc-400 hover:text-white'}`}
          >
            All Users ({items.length})
          </button>
          <button 
            onClick={() => setFilter('pending')}
            className={`px-3 py-1.5 rounded-xl transition-all flex items-center gap-1.5 ${filter === 'pending' ? 'bg-amber-500 text-black font-black' : 'bg-white/5 text-amber-400 hover:bg-amber-500/10'}`}
          >
            Pending Requests ({pendingUsers.length})
          </button>
          <button 
            onClick={() => setFilter('premium')}
            className={`px-3 py-1.5 rounded-xl transition-all flex items-center gap-1.5 ${filter === 'premium' ? 'bg-green-500 text-black font-black' : 'bg-white/5 text-green-400 hover:bg-green-500/10'}`}
          >
            Active Premium ({premiumCount})
          </button>
          <button 
            onClick={() => setFilter('standard')}
            className={`px-3 py-1.5 rounded-xl transition-all ${filter === 'standard' ? 'bg-zinc-700 text-white font-black' : 'bg-white/5 text-zinc-400 hover:text-white'}`}
          >
              ...
              >
                <div className="flex-1 space-y-2">
                  <div className="flex items-center gap-3">
                    <img src={u.photoURL} className={`w-10 h-10 rounded-full border ${u.isStrategist ? 'border-amber-500/40' : 'border-white/10 grayscale'}`} alt="" />
                    <div>
                      <div className="font-bold text-white leading-tight text-sm flex items-center gap-2 flex-wrap">
                        <span>{u.displayName}</span>
                        {u.isAdmin && <span className="text-[8px] bg-red-500/20 text-red-400 border border-red-500/30 px-1.5 py-0.5 rounded font-mono uppercase font-black tracking-widest">Admin</span>}
                        {u.isStrategist ? (
                          <span className="text-[9px] bg-amber-500/20 text-amber-300 border border-amber-500/30 px-2 py-0.5 rounded-md font-mono uppercase font-black tracking-wider flex items-center gap-1">
                            <Crown className="w-3 h-3 text-amber-400 fill-amber-400" /> Premium Active
                          </span>
                        ) : (
                          <span className="text-[8px] bg-zinc-800 text-zinc-400 border border-white/5 px-1.5 py-0.5 rounded font-mono uppercase font-black tracking-widest">Standard Free</span>
                        )}
                      </div>
                      <div className="text-[10px] text-zinc-400 uppercase font-bold tracking-widest font-mono pt-0.5">{u.email}</div>
                      {u.premiumRequestStatus && (
                        <div className="text-[9px] font-mono font-semibold text-zinc-400 pt-0.5">
                          Request status:{' '}
                          <span className={u.premiumRequestStatus === 'approved' ? 'text-green-400 font-bold' : u.premiumRequestStatus === 'pending' ? 'text-amber-400 font-bold animate-pulse' : 'text-red-400'}>
                            {u.premiumRequestStatus.toUpperCase()}
                          </span>
                        </div>
                      )}
                    </div>
                  ...
                  {u.premiumRequestPlan && (
                    <div className="mt-2 p-3 bg-zinc-950/80 rounded-xl border border-white/5 space-y-1 font-mono text-[9.5px]">
                      <div className="text-[9px] font-black uppercase tracking-widest text-amber-400 flex items-center gap-1.5">
                        Submitted Payment Details
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-0.5 text-zinc-400">
                        <div>Plan: <strong className="text-white">{u.premiumRequestPlan === 'elite' ? 'Elite Consult 1-on-1' : 'Sovereign Pass'}</strong></div>
                        <div>Method: <strong className="text-white">{(u.premiumRequestPaymentMethod || 'UPI').toUpperCase()}</strong></div>
                        <div className="sm:col-span-2">Details: <strong className="text-zinc-200">{u.premiumRequestDetails || 'Standard'}</strong></div>
                        {u.premiumRequestTransactionId && (
                          <div className="sm:col-span-2 text-amber-300">UTR / TXN ID: <strong className="text-white select-all">{u.premiumRequestTransactionId}</strong></div>
                        )}
                      </div>
                    </div>
                  )}
                ...
                {/* Direct Anytime Premium Toggle Switch / Controls */}
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 shrink-0 w-full sm:w-auto self-end md:self-center border-t md:border-t-0 border-white/10 pt-3 md:pt-0">
                  <div className="text-[9px] font-mono text-zinc-400 uppercase tracking-wider font-bold mb-1 sm:mb-0 hidden lg:block text-right pr-2">
                    Premium Authorization:
                  </div>

                  {/* Toggle Switch Buttons */}
                  <div className="flex items-center bg-zinc-950/80 border border-white/10 p-1 rounded-xl">
                    <button
                      type="button"
                      onClick={() => setPremiumAccess(u, true)}
                      className={`px-3 py-1.5 rounded-lg text-[10px] font-mono font-black uppercase tracking-wider transition-all flex items-center gap-1.5 cursor-pointer ${
                        u.isStrategist
                          ? 'bg-amber-500 text-black shadow-lg font-extrabold'
                          : 'text-zinc-400 hover:text-white hover:bg-white/5'
                      }`}
                      title="Activate Premium Access for this user at any time"
                    >
                      <Zap className={`w-3 h-3 ${u.isStrategist ? 'fill-black' : 'text-zinc-400'}`} />
                      <span>ON (Active)</span>
                    </button>
                    ...
                    <button
                      ...
                      title="Turn Off Premium Access for this user at any time"
                    >
                      <X className="w-3 h-3" />
                      <span>OFF (Revoke)</span>
                    </button>
                  </div>
                </div>
              </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-4">
          <button onClick={() => setEditing({})} className="px-4 py-2 bg-amber-500 text-black font-black uppercase tracking-wider text-xs rounded-xl flex items-center gap-2 hover:bg-amber-400 transition-all">
